...
class ConditionEvaluator:
    ...
    ALLOWED_FUNCTIONS = {
        ...
        "json_keys": lambda s: (
            list(_parse_llm_json(s).keys())  # type: ignore[union-attr]
            if _parse_llm_json(s) and isinstance(_parse_llm_json(s), dict)
            else []
        ),
        "json_values": lambda s: (
            list(_parse_llm_json(s).values())  # type: ignore[union-attr]
            if _parse_llm_json(s) and isinstance(_parse_llm_json(s), dict)
            else []
        ),
    ...
    def _eval_unaryop(self, node: ast.UnaryOp) -> Any:
        ...
        if isinstance(node.op, ast.Not):
            ...
            return self.ALLOWED_UNARY_OPERATORS[type(node.op)](operand)  # type: ignore[index]
        else:
