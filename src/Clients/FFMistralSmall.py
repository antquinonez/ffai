...
class FFMistralSmall(FFAIClientBase):
    def __init__(self, config: dict | None = None, **kwargs):
        ...
        for key, value in all_config.items():
            match key:
                case "api_key":
                    self.api_key = value or os.getenv("MISTRALSMALL_API_KEY")
                case "model":
                    self.model = value
                case "temperature":
        ...
        self.temperature = getattr(
            self,
            "temperature",
            float(os.getenv("MISTRALSMALL_TEMPERATURE", defaults["temperature"])),
        )
        self.max_tokens = getattr(
            self,
            "max_tokens",
            int(os.getenv("MISTRALSMALL_MAX_TOKENS", defaults["max_tokens"])),
        )
        self.system_instructions = getattr(
            self,
            "system_instructions",
            os.getenv("MISTRALSMALL_SYSTEM_INSTRUCTIONS", defaults["instructions"]),
        )
