...
class FFEmbeddings:
    ...
    async def async_embed(self, texts: str | list[str]) -> list[list[float]]:
        """Async version of embed().

        Uses litellm.aembedding for API models.
        Local models run synchronously (they release the GIL).
        ...
        return await self._async_embed_api(texts)

    async def _async_embed_api(self, texts: list[str]) -> list[list[float]]:
        """Generate embeddings using LiteLLM async API."""
        if not self.api_key:
            raise ValueError(f"No API key configured for model {self.model}")
        ...
        if uncached_texts:
            logger.debug(f"Generating async API embeddings for {len(uncached_texts)} texts")
    ...
    async def async_embed_single(self, text: str) -> list[float]:
        ...
        """
        return (await self.async_embed(text))[0]
