...
class TestNamedPromptInterpolation:
    ...
    def test_reference_in_multi_step_context(self, integration_client: FFAIClientBase):
        ...
        summary_interaction = ffai.history.get_latest_interaction_by_prompt_name("summary")
...
class TestNamedPromptHistory:
    def test_history_records_named_prompts(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: alpha", prompt_name="first")
        ffai.workflow.generate_response("Say: beta", prompt_name="second")

        history = ffai.history.get_prompt_attr_history()
    ...
    def test_ordered_history_preserves_sequence(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: one", prompt_name="step1")
        ffai.workflow.generate_response("Say: two", prompt_name="step2")
    ...
    def test_get_latest_by_name(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say exactly: first attempt", prompt_name="retry_me")
        ffai.workflow.generate_response("Say exactly: second attempt", prompt_name="retry_me")

        latest = ffai.history.get_latest_interaction_by_prompt_name("retry_me")
...
class TestNamedPromptUsageAndCost:
    def test_each_named_call_tracks_usage(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        r1 = ffai.workflow.generate_response("Say: hello", prompt_name="greet")
        r2 = ffai.workflow.generate_response("Say: world", prompt_name="place")
    ...
    def test_interpolated_call_has_higher_input_tokens(self, integration_client: FFAIClientBase):
        ...
        resolved = ffai.history.get_latest_interaction_by_prompt_name("fastest")
...
class TestResponseOptionsLive:
    ...
    def test_condition_true_executes(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: elephant", prompt_name="animal")
        result = ffai.workflow.generate_response(
    ...
    def test_condition_false_skips(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: cat", prompt_name="animal")
        result = ffai.workflow.generate_response(
