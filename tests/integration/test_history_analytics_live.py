...
class TestPersistRoundTripLive:
    def test_persist_and_reload_all_histories(self, integration_client: FFAIClientBase, tmp_path):
        ...
        ffai.workflow.generate_response("Say: hello", prompt_name="greet")
        ffai.workflow.generate_response("Say: goodbye", prompt_name="farewell")

        ffai._exporter.persist_dir = str(tmp_path)
        ffai._exporter.persist_name = "live_test"
        assert ffai.history.persist_all_histories() is True
    ...
    def test_persist_contains_usage_and_history_fields(self, integration_client: FFAIClientBase, tmp_path):
        ffai = FFAI(integration_client)
        r = ffai.workflow.generate_response("Say: test", prompt_name="dep_check")
    ...
    def test_search_after_persist_reload(self, integration_client: FFAIClientBase, tmp_path):
        ...
        ffai.workflow.generate_response("What is Rust?", prompt_name="rust_q")

        found = ffai.history.search_history(prompt_name="python_q")
        assert found.height == 1
        assert found["prompt_name"][0] == "python_q"

        found_rust = ffai.history.search_history(prompt_name="rust_q")
...
class TestSearchHistoryLive:
    def test_search_by_text_finds_match(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Explain quantum computing briefly", prompt_name="quantum")
        ffai.workflow.generate_response("Explain baking bread briefly", prompt_name="bread")

        found = ffai.history.search_history(text="quantum")
        assert found.height >= 1
    ...
    def test_search_by_time_range(self, integration_client: FFAIClientBase):
        ...
        ffai = FFAI(integration_client)
        before = time.time()
        ffai.workflow.generate_response("Say: alpha", prompt_name="time_a")
        ffai.workflow.generate_response("Say: beta", prompt_name="time_b")
        after = time.time()

        found = ffai.history.search_history(start_time=before, end_time=after + 1)
        assert found.height == 2

    def test_search_no_match_returns_empty(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: xyz", prompt_name="xyz")

        found = ffai.history.search_history(text="zzzzzz_nonexistent_keyword")
...
class TestCrossPromptAnalyticsLive:
    def test_response_length_stats_with_real_data(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: short", prompt_name="short_call")
        ffai.workflow.generate_response(
            "Write a three-sentence explanation of neural networks.",
            prompt_name="long_call",
        )

        stats = ffai.history.get_response_length_stats()
    ...
    def test_interaction_counts_by_date(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: date_test_1", prompt_name="d1")
        ffai.workflow.generate_response("Say: date_test_2", prompt_name="d2")

        counts = ffai.history.interaction_counts_by_date()
    ...
    def test_model_stats_matches_call_count(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: stat1", prompt_name="s1")
        ffai.workflow.generate_response("Say: stat2", prompt_name="s2")
        ffai.workflow.generate_response("Say: stat3", prompt_name="s3")

        stats = ffai.history.get_model_stats_df()
    ...
    def test_prompt_name_stats_counts(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.workflow.generate_response("Say: pns1", prompt_name="repeat_me")
        ffai.workflow.generate_response("Say: pns2", prompt_name="repeat_me")
        ffai.workflow.generate_response("Say: pns3", prompt_name="once")

        stats = ffai.history.get_prompt_name_stats_df()
        assert stats.height == 2
