...
class TestHistoryTrackingLive:
    def test_raw_history_records_all_calls(self, integration_client: FFAIClientBase):
        ...
        ffai.generate_response("Say: one", prompt_name="call1")
        ffai.generate_response("Say: two", prompt_name="call2")
        ffai.generate_response("Say: three", prompt_name="call3")

        history = ffai.history
    ...
    def test_prompt_attr_history_deduplicates_by_name(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Say: first", prompt_name="repeated")
        ffai.generate_response("Say: second", prompt_name="repeated")

        pah = ffai.get_prompt_attr_history()
        assert len(pah) == 2
    ...
    def test_clean_history_strips_whitespace(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Respond with lots of     spaces", prompt_name="spaces")

        clean = ffai.get_clean_interaction_history()
        assert len(clean) == 1
    ...
    def test_ordered_history_maintains_sequence(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Say: alpha", prompt_name="step_1")
        ffai.generate_response("Say: beta", prompt_name="step_2")
        ffai.generate_response("Say: gamma", prompt_name="step_3")
    ...
    def test_history_includes_model_and_usage(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Say: test", prompt_name="tracked")
...
class TestDataFrameExportLive:
    def test_history_to_dataframe(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Say: df_test_1", prompt_name="df1")
        ffai.generate_response("Say: df_test_2", prompt_name="df2")
    ...
    def test_statistics_dataframe(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        ffai.generate_response("Say: stats_test", prompt_name="stats_call")

        stats_df = ffai.get_model_stats_df()
...
class TestCostTrackingLive:
    def test_cost_accumulates_across_calls(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        r1 = ffai.generate_response("Say: cost1", prompt_name="cost1")
        r2 = ffai.generate_response("Say: cost2", prompt_name="cost2")
    ...
    def test_usage_is_nonzero(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        result = ffai.generate_response("What is 1+1?", prompt_name="usage_check")
    ...
    def test_longer_prompt_has_more_input_tokens(self, integration_client: FFAIClientBase):
        ffai = FFAI(integration_client)
        r_short = ffai.generate_response("Say: hi", prompt_name="short")
        ffai.clear_conversation()
        long_prompt = "Describe the following in one sentence: " + "x " * 200
        r_long = ffai.generate_response(long_prompt, prompt_name="long")
